...
class Tears:
    ...
    def draw(self):
        # self.sound.play()
        self.state = 'fire'
        # 16 and 10 added to centralise the tear
...
class Score:
    ...
    def show(self):
        showscore = self.font.render(
            f'Score: {str(self.value)}', True, (255, 255, 255))
...
while running:
    ...
    NewSpeed = (score.value//5)

    for i in range(len(enemy)):
        # enemy movement
        enemy[i].x += enemy[i].x_change
        if enemy[i].x < 0 and score.value < 5:
            enemy[i].x_change = 0.5
        elif enemy[i].x < 0 and score.value >= 5:
            enemy[i].x_change = NewSpeed
            enemy[i].y += enemy[i].y_change
        elif enemy[i].x > 736 and score.value < 5:
            enemy[i].x_change = -0.5
            enemy[i].y += enemy[i].y_change
        elif enemy[i].x > 736 and score.value >= 5:
            enemy[i].x_change = (-1)*int(NewSpeed)
            enemy[i].y += enemy[i].y_change
